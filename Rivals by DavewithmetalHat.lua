...
local Window = Rayfield:CreateWindow({
   ...
   LoadingSubtitle = "by DavewithmetalHat", -- Name Updated!
   ConfigurationSaving = {
      Enabled = true,
...
-- [[ VISUALS TAB - DAVE'S WORKING ESP ]]
    ...
    if _G.Aimbot and game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
        ...
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Team ~= LocalPlayer.Team and v.Character and v.Character:FindFirstChild(TargetPart) then
                ...
                if OnScreen then
                    -- Keeping the screen-centering fix
...
Rayfield:Notify({
   Title = "Script Loaded!",
   Content = "Credits updated to DavewithmetalHat. Good luck!",
   Duration = 5,
})
